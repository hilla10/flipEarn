...
const CredentialSubmission = ({ onClose, listing }) => {
  ...
  const handleAddField = () => {
    ...
    if (!name) toast('Please enter a field name');
  ...
  const handleSubmission = async (e) => {
    ...
    try {
      ...
      if (credential.length === 0) {
        return toast.error('Please add at least one filed');
      }
      ...
      <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-auto flex flex-col'>
        ...
        <div className='bg-linear-to-r from-indigo-600 to-indigo-400 text-white p-4 sm:rounded-t-lg flex items-center justify-between'>
          <div className='flex-1 min-w-0'>
            ...
            <p>
              Adding Credentials for {listing?.username} on{' '}
              {listing?.platform}{' '}
            </p>
          </div>
          ...
          <button
            onClick={onClose}
            className='className="ml-4 p-1 hover:bg-white/20 hover:bg-opacity-20 rounded-lg transition-colors"'>
            <X className='size-5' />
          </button>
        </div>
